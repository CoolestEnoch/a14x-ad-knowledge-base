# GraphBEV（通过Graph来更精确深度）

GraphDepth: Enhancing Accurate Depth Estimation via Graph matching for Bird's-Eye View Representation

###### 1.将以下代码复制到**view_transforms**（BEVFusion（Openpcdet））目录下并新建文件为**depth_lss_graph.py**
```python
import torch
...
###### 2.目录结构如下
![1706713369210-40a00c9f-f89d-4bca-95fe-71bd7002936e.png](./img/BOC_54AB8KcVgOvO/1706713369210-40a00c9f-f89d-4bca-95fe-71bd7002936e-128726.png)

###### 3.修改View_transforms中的__init__.py
```python
...
###### 4.重命名**bevfusion.ymal为bevfusion_graph.yaml**
在大约98行修改为如下

```yaml
VTRANSFORM:
        NAME: GraphDepthLSSTransform
        IMAGE_SIZE: [256, 704]
        IN_CHANNEL: 256
        OUT_CHANNEL: 80
        FEATURE_SIZE: [32, 88]
...
其中 ：Noise: True（控制是否增加噪声）

        K_graph: 25（邻居深度的数量）



> 更新: 2024-01-31 23:14:32  
> 原文: <https://3dcv.yuque.com/org-wiki-3dcv-mm1l0t/ysgfp9/pmdhup3shqwixzw1>