...
class Visualizer:
    ...
    def add_vis_long(self, index):
        ...
        if data['gt_ego_fut_cmd'][2] == 1:
            ...
            result1 = self.results[index + 2]
            result2 = self.results[index + 4]
            #import pdb;pdb.set_trace()
            bev_gt_path, bev_pred_path = self.bev_render.render_long(data, data1, data2, result, result1, result2, index)
            cam_pred_path = self.cam_render.render(data, result, index)
            #self.combine(bev_gt_path, bev_pred_path, cam_pred_path, index)
