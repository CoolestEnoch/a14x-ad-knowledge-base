...
class Visualizer:
    ...
    def add_vis_long(self, index):
        ...
        if data['gt_ego_fut_cmd'][2] == 1:
            ...
            key_list = list(self.results['plan_results']) # 0-based index
            result = self.results['plan_results'][key_list[index]]
            result1 = self.results['plan_results'][key_list[index + 2]]
            result2 = self.results['plan_results'][key_list[index + 4]]
            #import pdb;pdb.set_trace()
            bev_gt_path, bev_pred_path = self.bev_render.render_long(data, data1, data2, result, result1, result2, index, type ="vad")
            cam_pred_path = self.cam_render.render(data, result, index)
            #self.combine(bev_gt_path, bev_pred_path, cam_pred_path, index)
